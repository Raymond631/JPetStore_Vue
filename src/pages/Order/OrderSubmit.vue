    ...
    <div class="head_background">
      <div class="head_box">
        <a class="head_left_a" href="/jpetstore">
          <img alt="" class="head_left_p" src="../../assets/image/login/placeholder.png">
        </a>
        <h1 class="head_h1">确认订单</h1>
        <div class="head_right">
          <a class="head_right_in" href="/jpetstore/MyOrder.html">我的订单</a>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="order_body_background">
        ...
        <div class="order_lien_in order_in">
          ...
          <div class="order-address">
            <template v-if="!isEditMode">
              ...
              <p>
                <button class="order_lien_in" @click="toggleEditMode">编辑</button>
              </p>
            </template>
            <template v-else>
              <input v-model="editedAddress.receiverName" type="text" placeholder="姓名" />
              <input v-model="editedAddress.receiverPhone" type="tel" placeholder="联系方式" />
              <input v-model="editedAddress.receiverAddress" type="text" placeholder="收货地址" />
              <p>
      ...
      <div class="order_line order_line_bordtr">
        ...
        <div class="order-payment">
          <p v-for="(option, index) in paymentOptions" :key="index" :class="{ active: payIndex === index }" @click="selectPayment(index)">{{ option }}</p>
        </div>
...
<!--        <div class="order_lien_in"></div>-->
      ...
      <div class="order_line order_line_bordtr">
        ...
        <div class="order_lien_in">
          <a href="/jpetstore/MyCart.html">
            <span class="order_span_rigth" style="color: #ff6700">返回购物车→</span>
          </a>
        </div>
        <div class="order_lien_in"></div>
      </div>
      ...
      <div class="order_line order_line_bordtr">
        <div class="order_lien_in">
          <p class="order_price_11"><br>应付总额:<span class="order-cost">{{ cost }}</span>元</p>
        </div>
...
<!--        </div>-->
      ...
      <div class="pay">
        <button class="order-btn" :style="{ background: btnBackground }" @click="placeOrder">{{ btnText }}</button>
      </div>
    </div>
  </div>
  </div>
...
export default defineComponent({
  setup(){
    ...
    //保存地址
    const saveAddress = () => {
      if (editedAddress.receiverName && editedAddress.receiverPhone && editedAddress.receiverAddress){
        const data = {
