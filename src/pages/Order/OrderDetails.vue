<template>
  <div class="order_head">
    <div class="head_background">
      <div class="head_box">
        <a href="/jpetstore" class="head_left_a"
          ><img
  ...
  <div class="order_body">
    <div class="order_body_background">
      ...
      <div class="order_line order_line_bordtr">
        ...
        <div class="order_lien_in">
          <p class="order_PStime" style="font: 18px 'Microsoft YaHei'">
            {{ orderId }}
          </p>
        </div>
      </div>
      ...
      <div class="order_line order_line_bordtr">
        <p
          class="order_price"
          style="margin-top: 20px; width: 200px; padding-left: 40px"
        >
          宠物
        </p>
        <p
      ...
      <div class="order_items">
        <div
          ...
          v-for="item in order.orderItemDOList"
          :key="item.itemId"
        >
          <div class="order_lien_in">
            <img
      ...
      <div class="order_line order_line_bordtr">
        ...
        <div class="order_price1">
          <p class="order_price_2">
            <span id="cost" style="font-size: 35px">{{ order.orderCost }}</span
            >元
          </p>
        </div>
      </div>
      <!-- 返回订单列表-->
      <div class="pay">
        <router-link to="/MyOrder" class="pay_a" id="order_submit"
          >返回订单列表</router-link
        >
      </div>
    </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const getDetails = () => {
      axios
        .get(`http://localhost:8080/jpetstore/orders/${orderId.value}`)
        .then((res) => {
          order.value = res.data.data;
          console.log(order.value);
          for (let i = 0; i < order.value.orderItemDOList.length; i++) {
            order.value.orderItemDOList[
              i
            ].itemImage = `http://localhost:8080/jpetstore/image/look/${order.value.orderItemDOList[i].itemImage}`;
          }
        })
        .catch((err) => {
          console.error(err);
